...
def main():
    ...
    for wid in test_wids:
        # Reset per-well state — prevents the previous iteration's `tw_tr`
        # from leaking into this well via `dir()` inspection (P0-2 in v1).
        tw_tr = None
        hw_tr = None
    ...
    # P0-1 fix: hard-align to sample_submission so any missing well/row
    # gets filled with the median of submitted preds rather than dropped.
    ...
    if n_missing:
        ...
        print(f"⚠ {n_missing} rows missing from PF preds → filling with median {fill_val:.2f}")
