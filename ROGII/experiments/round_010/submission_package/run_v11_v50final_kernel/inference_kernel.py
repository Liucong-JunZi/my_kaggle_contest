"""ROGII run_v11 inference kernel: run_v7 full ensemble + v50 finalizer."""
# %%
...
OUT_PATH  = "/kaggle/working/submission.csv" if _find_kaggle_input() else "/Users/liucong/code/kaggle/ROGII/results/round_010/submission_inference_v11_v50final.csv"
...
V50_DIR = _find_dataset_input(
    "v50-wellbore-phasea-artifacts",
    "annoyingbrine/v50-wellbore-phasea-artifacts",
    local="/Users/liucong/code/kaggle/ROGII/experiments/public_resources/datasets_raw/annoyingbrine_v50-wellbore-phasea-artifacts/v50_wellbore-phaseA-artifacts",
)

print(f"INPUT_DIR = {INPUT_DIR}")
print(f"RAVAGHI_DIR = {RAVAGHI_DIR}")
print(f"V10_DIR = {V10_DIR}")
print(f"V50_DIR = {V50_DIR}")
...
assert V50_DIR and os.path.isdir(V50_DIR), "V50_DIR missing"

ROLLING_WINS = [5, 21, 51, 101]

RUN_V11_WEIGHTS = {
    "c20_r9_pf128_full": 0.4556,
    "c97_chesnikov_v50_hc_pp": 0.2878,
    "v10_tabicl_A": 0.1424,
    "c54_ravaghi_cat2": 0.0481,
    "c50_ravaghi_lgb1": 0.0174,
    "c52_ravaghi_lgb3": 0.0159,
    "v10_tabicl_B": 0.0101,
}
RAVAGHI_MODELS = {
...
def load_v50_offset(test):
    candidates = [Path(V50_DIR) / "submission.csv"]
    if os.path.isdir(V50_DIR):
        candidates.extend(sorted(Path(V50_DIR).glob("*/submission.csv")))
    sub_path = next((p for p in candidates if p.exists()), None)
    if sub_path is None:
        raise FileNotFoundError(f"v50 submission.csv not found under {V50_DIR}; checked={candidates[:5]}")
    print(f"  v50 submission: {sub_path}")
    v50 = pd.read_csv(sub_path)
    if list(v50.columns) != ["id", "tvt"]:
        raise ValueError(f"unexpected v50 submission columns: {v50.columns.tolist()}")
    v50["well"] = v50["id"].str.rsplit("_", n=1).str[0]
    v50["row_idx"] = v50["id"].str.rsplit("_", n=1).str[1].astype("int32")

    base = test[["well", "row_idx", "last_known_tvt"]].copy()
    base["id"] = base["well"] + "_" + base["row_idx"].astype(str)
    aligned = base.merge(v50, on=["well", "row_idx", "id"], how="left", validate="one_to_one")
    missing = int(aligned["tvt"].isna().sum())
    print(f"  v50 alignment: rows={len(aligned):,} missing={missing}")
    if missing:
        raise ValueError(f"missing v50 predictions for {missing} rows")
    out = (aligned["tvt"].to_numpy(np.float64) - aligned["last_known_tvt"].to_numpy(np.float64)).astype(np.float32)
    print(f"    c97_chesnikov_v50_hc_pp mean={out.mean():.3f} std={out.std():.3f} range=[{out.min():.1f},{out.max():.1f}]")
    return {"c97_chesnikov_v50_hc_pp": out}
...
def main():
    t0 = time.time()
    print("=== ROGII run_v11: run_v7 full ensemble + v50 finalizer ===\n")
    ...
    for label, path in [("RAVAGHI_DIR", RAVAGHI_DIR), ("V10_DIR", V10_DIR), ("V50_DIR", V50_DIR)]:
    ...
    print(f"  ✓ v50 finalizer artifacts: {V50_DIR}")
    ...
    print(f"\n[4/4] Generating run_v11 predictions")
    t3 = time.time()

    # Every component is in offset space (target = TVT - last_known_tvt), matching
    # the OOF candidates used by run_v11_submitready_v7_plus_v50final.
    # The final submission is absolute TVT, so add last_known_tvt once below.
    components = {
    ...
    components.update(load_v50_offset(test))

    wsum = float(sum(RUN_V11_WEIGHTS.values()))
    print(f"  raw run_v11 weight sum: {wsum:.6f}")
    blend_off = np.zeros(len(test), dtype=np.float64)
    for cid, w in RUN_V11_WEIGHTS.items():
        if cid not in components:
            raise KeyError(f"missing component: {cid}")
        arr = components[cid].astype(np.float64)
        blend_off += (w / wsum) * arr
        print(f"    {cid:28s} w={w/wsum:.4f} mean={arr.mean():.3f} std={arr.std():.3f}")
