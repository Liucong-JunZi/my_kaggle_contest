"""ROGII run_v12 inference kernel: hidden-safe PF128 + ravaghi ensemble."""
# %%
...
OUT_PATH  = "/kaggle/working/submission.csv" if _find_kaggle_input() else "/Users/liucong/code/kaggle/ROGII/results/round_010/submission_inference_v12_hidden_safe.csv"
...
RUN_V12_WEIGHTS = {
    "c20_r9_pf128_full": 0.5291714916147652,
    "c51_ravaghi_lgb2": 0.1535731451526435,
    "c54_ravaghi_cat2": 0.11175844086902667,
    "c50_ravaghi_lgb1": 0.0879452049930177,
    "c53_ravaghi_cat1": 0.05929910702037566,
    "c52_ravaghi_lgb3": 0.03759614717512768,
}
RAVAGHI_MODELS = {
...
def main():
    t0 = time.time()
    print("=== ROGII run_v12: hidden-safe PF128 + ravaghi ensemble ===\n")
    ...
    for label, path in [("RAVAGHI_DIR", RAVAGHI_DIR)]:
    ...
    print("  ✓ hidden-safe: no public-test precomputed v10/v50 CSVs used")
    ...
    print(f"\n[4/4] Generating run_v12 hidden-safe predictions")
    t3 = time.time()

    # Every component is in offset space (target = TVT - last_known_tvt), matching
    # the OOF candidates used by run_v12_hidden_safe_pf_ravaghi.
    ...
    wsum = float(sum(RUN_V12_WEIGHTS.values()))
    print(f"  raw run_v12 weight sum: {wsum:.6f}")
    blend_off = np.zeros(len(test), dtype=np.float64)
    for cid, w in RUN_V12_WEIGHTS.items():
