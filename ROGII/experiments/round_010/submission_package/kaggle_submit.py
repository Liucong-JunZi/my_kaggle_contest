"""ROGII Kaggle Notebook submission — pure 128-seed PF (no ML).

Self-contained script: PF kernel + main pipeline merged into a single file
so Kaggle's `kernel_type: script` can run it without external modules.

Pipeline per test well:
  1. Load horizontal + typewell CSVs.
  2. If the well also exists in train/, override TVT_input from train and
     project TVT directly from the formation-contact column (deterministic).
  3. Otherwise: 128-seed × 500-particle × 4-scale PF ensemble + 14-config
     beam ensemble + 6-bin per-well selector + 4th-degree robust polynomial
     projection.

Output: /kaggle/working/submission.csv with columns id, tvt (absolute TVT, ft).

Wall time: ~4–6 h for ~100 hidden test wells on Kaggle CPU notebook.
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# PF kernel (inlined from training_code/pf_128seed.py)
# ═══════════════════════════════════════════════════════════════════════════════
PF_N_PARTICLES = 500
PF_N_SEEDS     = 128
PF_SCALES      = (3.0, 5.0, 8.0, 12.0)
PF_INIT_SPREAD = 3.0   # wide initial particle spread → robust at PS
...
def run_particle_filter(hw, tw, n_particles=PF_N_PARTICLES, seed=42):
    """Single-seed conservative PF.

    Returns (out_vals, log_lik) where out_vals is length len(hw) (known rows
    copied from TVT_input, lateral rows hold the PF posterior mean), and
    log_lik is the total GR-observation log-evidence over the lateral.
    """
    ...
    # GR interpolated *before* PF — fills NaN gaps so the filter always has obs.
...
def run_pf_lik_ensemble_scales(hw, tw, n_particles=PF_N_PARTICLES, n_seeds=PF_N_SEEDS):
    """128-seed log-likelihood-weighted PF ensemble across PF_SCALES.

    Returns dict with keys "pf_scale_3" / "pf_scale_5" / "pf_scale_8" /
    "pf_scale_12" / "pf_mean", each a length-len(hw) array.
    """
    preds = []
    liks  = []
...
def project_lateral(tvt_pred, eval_mask):
    ...
    for wid in test_wids:
        ...
        if wid in train_wids:
            try:
                ...
                # Defensive override: ensure PF + beam see the same known segment
                # as the physical model. Both cols still have NaN on the lateral.
