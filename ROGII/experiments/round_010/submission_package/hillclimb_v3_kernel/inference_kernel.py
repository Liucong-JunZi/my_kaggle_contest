...
# Model directory - try multiple approaches
if _find_kaggle_input():
    # Try mounted dataset first
    MODEL_DIR = "/kaggle/input/rogii-pretrained-models-v2"
    if not os.path.exists(MODEL_DIR) or not os.path.isdir(MODEL_DIR):
        # Fallback to kagglehub download
        print(f"[DEBUG] Mounted dataset not found, trying kagglehub download...")
        import kagglehub
        MODEL_DIR = kagglehub.dataset_download("smartorz/rogii-pretrained-models-v2")
        print(f"[DEBUG] Downloaded to: {MODEL_DIR}")
else:
    MODEL_DIR = "/Users/liucong/code/kaggle/ROGII/experiments/round_010/submission_package/hillclimb_v3_kernel"
...
def main():
    ...
    # Debug: Check dataset paths
    print(f"\n[DEBUG] Checking MODEL_DIR: {MODEL_DIR}")
    dataset_exists = os.path.exists(MODEL_DIR)
    dataset_isdir = os.path.isdir(MODEL_DIR) if dataset_exists else False
    print(f"[DEBUG]   exists={dataset_exists}, is_dir={dataset_isdir}")

    if dataset_exists and dataset_isdir:
        try:
            files = os.listdir(MODEL_DIR)
            print(f"[DEBUG]   Files in {MODEL_DIR}:")
            for f in sorted(files):
                fpath = os.path.join(MODEL_DIR, f)
                size = os.path.getsize(fpath)
                readable = os.access(fpath, os.R_OK)
                print(f"[DEBUG]     - {f} ({size} bytes), readable={readable}")
        except Exception as e:
            print(f"[DEBUG]   [ERROR listing {MODEL_DIR}: {e}]")
    else:
        # Try alternative dataset path
        alt_paths = [
            '/kaggle/input/rogii-pretrained-models-v1',
            '/kaggle/input/smartorz/rogii-pretrained-models-v1',
            '/kaggle/input/rogii-pretrained-models-v2',
        ]
        for alt_path in alt_paths:
            if os.path.exists(alt_path) and os.path.isdir(alt_path):
                print(f"[DEBUG] Found alternative dataset at: {alt_path}")
                try:
                    files = os.listdir(alt_path)
                    print(f"[DEBUG]   Files: {files}")
                    for f in files:
                        fpath = os.path.join(alt_path, f)
                        size = os.path.getsize(fpath)
                        print(f"[DEBUG]     - {f}: {size} bytes")
                except Exception as e:
                    print(f"[DEBUG]   [ERROR listing {alt_path}: {e}]")
    ...
    print(f"\n[2/4] Loading pre-trained models from {MODEL_DIR}")
    t1 = time.time()
    # Load LightGBM
    lgb_path = os.path.join(MODEL_DIR, "lgb_model.txt")
    model_lgb = lgb.Booster(model_file=lgb_path)
    print(f"  ✓ LGB loaded from {lgb_path}")

    # Load CatBoost
    cat_path = os.path.join(MODEL_DIR, "cat_model.cbm")
    model_cat = CatBoostRegressor()
    model_cat.load_model(cat_path)
    print(f"  ✓ CAT loaded from {cat_path}")

    # Load feature columns
    feat_path = os.path.join(MODEL_DIR, "feat_cols.pkl")
    with open(feat_path, "rb") as f:
        feat_cols = pickle.load(f)
    print(f"  ✓ feat_cols loaded ({len(feat_cols)} features)")
    print(f"  Model load time: {time.time()-t1:.0f}s")

    print(f"\n[3/4] Building TEST features")
    t2 = time.time()
    ...
    # Hill climb v3 blend (per run_v3_with_c20-23.json averaged weights):
    #   c20_r9_pf128_full  0.7785  ← PF 128-seed dominant
    #   p14_cat            0.1089
    #   p14_lgb            0.0926
    #   p5_lgb             0.0131  (skipped — same architecture as p14_lgb,
    #                                rolled into p14_lgb)
    #   c06_xgb_default    0.0069  (skipped — no XGB model checkpoint here;
    #                                 0.7% weight is in the noise)
    # We renormalize the 3 retained components: PF / CAT / LGB.
    W_PF, W_CAT, W_LGB = 0.7785, 0.1089, 0.0926 + 0.0131  # 0.1057
    Wsum = W_PF + W_CAT + W_LGB
    W_PF, W_CAT, W_LGB = W_PF / Wsum, W_CAT / Wsum, W_LGB / Wsum

    # All three are predicting in offset space (target = TVT - last_known_tvt).
    # PF candidate stored absolute pf_ens_s12 -> convert to offset.
    pf_ens_off = test["pf_ens_s12_abs"].values - test["last_known_tvt"].values
    blend_off = W_PF * pf_ens_off + W_CAT * pred_cat + W_LGB * pred_lgb
    pred_tvt = test["last_known_tvt"].values + blend_off

    print(f"  weights: PF128={W_PF:.4f}  CAT={W_CAT:.4f}  LGB={W_LGB:.4f}")
    print(f"  PF_ens_s12 off mean: {pf_ens_off.mean():.3f}  std: {pf_ens_off.std():.3f}")
    print(f"  pred_lgb off mean: {pred_lgb.mean():.3f}  pred_cat off mean: {pred_cat.mean():.3f}")
    print(f"  blend mean: {pred_tvt.mean():.0f}  range: [{pred_tvt.min():.0f}, {pred_tvt.max():.0f}]")
