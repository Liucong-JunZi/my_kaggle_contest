...
def extract_well_features(wid, data_dir):
    ...
    for k, r in enumerate(lat_idx):
        rec = {
            ...
            # Keep absolute pf_ancc for the v8 heuristic blend (not used as model feature)
...
def main():
    t0 = time.time()
    print("=== ROGII v8: v4 baseline (36 feat, no multi-PN, no spatial) + PF heuristic blend ===\n")
    ...
    # v8: keep v4 36-feat stack — drop pf_ancc_abs (heuristic-only, not a feature)
    feat_cols = [c for c in train.columns if c not in {
        "well","row_idx","target","pf_ancc_abs",
    }]
    ...
    # v4 production blend: 0.4·LGB + 0.6·CAT
    ml_off = 0.4 * pred_lgb + 0.6 * pred_cat
    ml_tvt = test["last_known_tvt"].values + ml_off
    pf_tvt = test["pf_ancc_abs"].values
    # v8 heuristic blend: 0.75·ML + 0.25·PF_ancc (CV per-well 9.91 / flat 12.95
    # vs v4 ML alone 10.07 / 13.01; mirrors LB-7.776 kernel's 0.3·ridge + 0.7·heuristic
    # design but with our weaker PF in minority share)
    pred_tvt = 0.75 * ml_tvt + 0.25 * pf_tvt
    print(f"  ML mean: {ml_tvt.mean():.0f}, PF mean: {pf_tvt.mean():.0f}, blend mean: {pred_tvt.mean():.0f}")
    sub = pd.DataFrame({
